...
func (attributesFirmwareSetL) LoadFirmwareSet(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAttributesFirmwareSet interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeAttributesFirmwareSet.(*AttributesFirmwareSet)
		if !ok {
			object = new(AttributesFirmwareSet)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeAttributesFirmwareSet)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeAttributesFirmwareSet))
			}
		}
	} else {
		s, ok := maybeAttributesFirmwareSet.(*[]*AttributesFirmwareSet)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeAttributesFirmwareSet)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeAttributesFirmwareSet))
			}
		}
	}
...
func (o *AttributesFirmwareSet) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"attributes_firmware_set\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *AttributesFirmwareSet) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return AttributesFirmwareSetExists(ctx, exec, o.ID)
}
