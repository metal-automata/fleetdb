...
var (
	biosConfigComponentDBTypes = map[string]string{`ID`: `uuid`, `FKBiosConfigSetID`: `uuid`, `Name`: `text`, `Vendor`: `text`, `Model`: `text`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`}
	_                          = bytes.MinRead
)
