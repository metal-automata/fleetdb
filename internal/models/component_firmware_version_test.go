...
var (
	componentFirmwareVersionDBTypes = map[string]string{`ID`: `uuid`, `Component`: `text`, `Vendor`: `text`, `Model`: `ARRAYtext`, `Filename`: `text`, `Version`: `text`, `Checksum`: `text`, `UpstreamURL`: `text`, `RepositoryURL`: `text`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `InstallInband`: `boolean`, `Oem`: `boolean`}
	_                               = bytes.MinRead
)
